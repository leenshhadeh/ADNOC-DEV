 ...
 * specific field. Reviewer (QM / BPA PM) can view past comments and add new ones.
 *
 * Adapted from the assessment-data module's FieldCommentSheet to work with
 * automation-targets processId + fieldId params.
 */
...
const FieldCommentSheet = ({ open, onOpenChange, processId, fieldId }: FieldCommentSheetProps) => {
  ...
  if (!hasField) {
    return (
      <div className="flex w-[400px] shrink-0 flex-col rounded-3xl bg-accent p-4">
        ...
        <p className="mt-8 text-center text-sm text-[#687076]">
  ...
  const handleSubmit = () => {
    ...
    if (!trimmed || !processId || !fieldId) return
    addComment({ processId, fieldId, text: trimmed })
    setNewComment('')
  }
