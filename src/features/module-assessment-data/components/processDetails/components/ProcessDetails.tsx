...
const ProcessDetails = (props: any) => {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      {data?.map((item: any) => (
        <div key={item.label} className="border-l border-gray-300 first:ps-0 px-4 first:border-l-0">
          <span className="text-muted-foreground text-sm">{item.label} {item.stageCurrent?`(${item.stageCurrent}/${item.stageTotal})`:''}</span>
          {/* value: */}
          <div className="flex">
            {item.canCopy ? (
