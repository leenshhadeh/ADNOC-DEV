...
// ── Pure helpers (module-level so they can be reused by the export hook) ──────
...
const formatSharedService = (value: SharedService): string => {
  if (!value) return ''
  if (typeof value === 'string') return value
  // Preserve the object shape as a JSON string so getSharedCellValue can parse it back
  return JSON.stringify({ services: value.services, shared: value.shared })
}
...
const ProcessDataTable = ({
  ...
  const getSharedCellValue = (item: string) => {
    let parsedValue: { services?: number; shared?: number }
    try {
  ...
  const columns = useMemo<ColumnDef<FlatAssessmentRow, unknown>[]>(
    () => [
      ...
      },
      {
        ...
        meta: { pinnedCol: true },
        cell: (info) => {
          const isBulkModeActive = info.table.options.meta?.isBulkMode ?? false
          const isSelected = info.row.getIsSelected()
          const l3Value = info.getValue<string>()
          const hasL4 = !!info.row.original.l4
          return (
            <div className="flex items-center justify-between gap-2">
              <div className="flex flex-col gap-0.5">
                <span className="text-foreground text-sm font-medium">{l3Value}</span>
                <span className="text-muted-foreground text-xs">
                  {l3Value ? info.row.original.l3Code : ''}
                </span>
              </div>
              {!isBulkModeActive && <CellMenuOptions item={info.row.original} />}
              {isBulkModeActive && !hasL4 && (
                <Checkbox
                  checked={isSelected}
                  onCheckedChange={(checked) => info.row.toggleSelected(!!checked)}
                  aria-label={`Select ${l3Value}`}
      ...
      },
      {
        ...
        cell: (info) => {
          const isBulkModeActive = info.table.options.meta?.isBulkMode ?? false
          const isSelected = info.row.getIsSelected()
          const l4Value = info.getValue<string>()
          return (
            <div className="flex items-center justify-between gap-2">
              <div className="flex flex-col gap-0.5">
                <span className="text-foreground text-sm font-medium">{l4Value}</span>
                <span className="text-muted-foreground text-xs">
                  {l4Value ? info.row.original.l4Code : ''}
                </span>
              </div>
              {!isBulkModeActive && l4Value && <CellMenuOptions item={info.row.original} />}
              {isBulkModeActive && l4Value && (
                <Checkbox
                  checked={isSelected}
                  onCheckedChange={(checked) => info.row.toggleSelected(!!checked)}
                  aria-label={`Select ${l4Value}`}
                  className="ms-auto shrink-0"
                  onClick={(e) => e.stopPropagation()}
                />
              )}
